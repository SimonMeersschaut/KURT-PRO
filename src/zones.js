...
*/
function createZoneCard(zoneId, date){
    var zoneName = "Arenberg - Zolder";

    const dateString = date.getFullYear() + "-" + (date.getMonth()+1) + "-" + date.getDate();
    fetch("https://kurt3.ghum.kuleuven.be/api/resourcetypeavailabilities?locationId=1&zoneId="+zoneId.toString()+"&resourceTypeId=302&pageNumber=0&startDate="+dateString+"&startTime=&endDate="+dateString+"&endTime=&participantCount=1&tagIds=&exactMatch=true&onlyFavorites=false&resourceNameInfix=&version=2.0")
    .then(resp => resp.json())
    .then(data => {
